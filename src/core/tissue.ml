...
let cell cytoplasm nucleus =
  { cytoplasm;
    nucleus;
  }

let to_item =
  Item.(   (* (cytoplasm, nucleus), (clot, is_active) *)
...
let to_init_item =
  Item.InitValue.(   (* (cytoplasm, nucleus), is_active *)
...
let init_items o =
  match o.init with
  | None   ->  Matrix.empty 0 0 Item.InitValue.Out 
